...
function displayDecoupled($merchant_id, $merchant_name)
{
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr class='clickable-row' data-href='decoupled_settlement_report.php?decoupled_report_id=" . $row['decoupled_report_id'] . "&merchant_id=" . $merchant_id . "&merchant_name=" . urlencode($merchant_name) . "&settlement_period_start=" . urlencode($row['settlement_period_start']) . "&settlement_period_end=" . urlencode($row['settlement_period_end']) . "&bill_status=" . urlencode($row['bill_status']) .  "'>";
            echo "<td style='text-align:center;'>" . $row['settlement_number'] . "</td>";
            echo "<td style='text-align:center;'><i class='fa-solid fa-file-pdf' style='color:#4BB0B8'></i> " . $row['merchant_brand_name'] . " - " . $row['settlement_period']. " - (" . $row['settlement_number'] . ").pdf</td>";
...
<head>
    ...
    <title><?php echo htmlspecialchars($merchant_name); ?> - Decoupled Settlement Reports</title>
