DROP PROCEDURE IF EXISTS decoupled_store_pretrial;

DELIMITER //

CREATE PROCEDURE decoupled_store_pretrial(
...
BEGIN
    ...
    store_details AS (
        SELECT DISTINCT
            ...
            store.store_name AS store_brand_name,
            store.store_address AS business_address,
            merchant.merchant_id,
            merchant.merchant_name,
            fee.commission_type
        FROM `store_view` store
    ...
    SELECT 
        ...
        t.`Store ID` AS store_id,
        s.store_business_name,
        s.store_brand_name,
        s.business_address,
    ...
    JOIN store_details s ON t.`Store ID` = s.store_id
    WHERE t.`Store ID` = "', store_id, '"
    GROUP BY 
        t.`Store ID`, 
        s.store_business_name, 
        s.store_brand_name, 
