...
BEGIN
    ...
    SET @sql_insert = CONCAT('INSERT INTO report_history_coupled 
	    ...
	    ROUND(SUM(`Total Billing`) / 1.12 * 0.02, 2) AS cwt_from_transaction_fees,
            ROUND(SUM(`PG Fee Amount`) / 1.12 * 0.02, 2) AS cwt_from_pg_fees,
	...
	JOIN
	    ...
	    AND (`Bill Status` = ''PRE-TRIAL'' OR `Bill Status` = ''BILLABLE'')
        GROUP BY 
            `Merchant ID`');
