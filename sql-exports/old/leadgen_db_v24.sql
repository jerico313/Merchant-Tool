...
CREATE DEFINER=`root`@`localhost` PROCEDURE `gcash_merchant` (IN `merchant_id` VARCHAR(36), IN `start_date` DATE, IN `end_date` DATE)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `gcash_store` (IN `store_id` VARCHAR(36), IN `start_date` DATE, IN `end_date` DATE)   BEGIN
...
CREATE TABLE `fee` (
  ...
  `is_cwt_rate_computed` tinyint(1) NOT NULL DEFAULT 0,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TRIGGER `update_fee_value` AFTER UPDATE ON `fee` FOR EACH ROW BEGIN
  ...
  IF OLD.is_cwt_rate_computed != NEW.is_cwt_rate_computed THEN
    INSERT INTO fee_history (fee_history_id, fee_id, column_name, old_value, new_value)
    VALUES (UUID(), NEW.fee_id, 'is_cwt_rate_computed', OLD.is_cwt_rate_computed, NEW.is_cwt_rate_computed);
  END IF;
END
$$
DELIMITER ;
...
,`Is CWT Rate Computed` varchar(3)
